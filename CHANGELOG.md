...
v1.11.0 完成了"工具强制 + 规则瘦身"的结构性重构,但在真实场景触发下暴露出 **SKILL.md 入口层仍可被软化绕开** 的问题:

- 用户主动 `/spec-power` 调用后, Claude 把 SKILL.md 当"索引手册"翻阅, 跳过变更目录初始化、直接用对话消息代替 `proposal.md` / `design.md` 产物
- 项目 `CLAUDE.md` 若声明 "不写计划文档 / 不用 Plan Mode" 之类精简约束, Claude 默认偏向项目侧, Standard 模式退化为纯对话讨论
- 铁律速查表仍用"描述性"语气 ("适用模式 / 权威出处"), 缺少禁止信号, Claude 实际执行时自行软化
- `TaskCreate` 工具与 `tasks.md` 产物职责不清, 容易用前者代替后者
- 模式判定只要求"展示推理"但没给强制格式, 实际只输出一句 "Standard 模式" 了事

v1.12.0 聚焦 **入口层纪律强化**: 把 SKILL.md 从"可选参考"变成"不可绕过的指令入口"。

### 新增 ✨

- **🚨 收到任务后立即执行 (硬入口)** — SKILL.md 开头新增指令式启动清单
  - Step 0: 恢复检测 (in-progress 变更扫描)
  - Step 1: 模式判定 + **必须向用户展示推理**
  - Step 2: 按模式立即执行分支动作 (Flow / Standard / Strict 各自的产物要求)
  - Step 3: 产物锚点 (proposal.md / design.md / tasks.md / reviews/) — 未落盘禁止进下一阶段
  - 4 条反模式示例帮助识别常见跳过路径

- **优先级声明 (冲突覆盖规则)** — 新增小节明确:
  - 用户主动调用 `/spec-power` 意味着选择结构化流程
  - SpecPower 产物要求**覆盖** CLAUDE.md / 项目级指令的"不写计划文档 / 不用 Plan Mode / 直接编码"类精简约束
  - 可降级回项目默认的 2 种情况: 用户明确放弃 / Flow 模式
  - 项目编码规范 (命名/目录/变体隔离) 与产物要求冲突时前者优先, 但不抵消产物落盘要求

- **任务管理职责划分章节** — `tasks.md` vs `TaskCreate` 对照表
  - 7 维度对比 (性质 / 生命周期 / 必需性 / 更新方式 / 与 yaml 关系 / 驱动循环等)
  - 明确 `tasks.md` 为"真相之源", `TaskCreate` 仅作会话级 UI 辅助
  - 列出 3 条常见反模式 (纯 `TaskCreate` / 编号不一致 / 状态单向同步丢失)

- **模式判定输出格式 (强制模板)** — 新增 MUST 级推理模板
  - 8 个观察维度: 影响文件数 / 跨模块程度 / 需求清晰度 / 新接口 / 协作维度 / 初判信号 / 边界验证 / 降升档
  - 模板要求展示 "推荐 + 理由 + 用户可改" 三段信息
  - 违反模板 → 违反 R9 → 按 discipline-recovery.md 处置

### 重构 🔨

- **关键规则速查表改禁止性语言** — 从描述性改为强制约束
  - 旧版: "规则 / 适用模式 / 权威出处" 三列, 语气温和
  - 新版: 9 条铁律 R1–R9, 每条以 "**禁止**..." 开头, 新增"违规即时动作"列
  - 标题从 "必须遵守" 改为 "铁律 / MUST"
  - 明确"规则退役必须走 discipline-recovery.md 正式流程, 禁止自行判断本次可跳过"

### 修复 🐛

- **消除 SKILL.md 被软化跳过的入口级漏洞** — v1.11.0 瘦身后的 163 行主文件缺少"不可绕过的第一动作", Claude 可以直接进对话模式; v1.12.0 把硬入口放在正文开头, 所有后续章节都从属于它

### 影响评估

- **向后兼容** ✅ — 所有 v1.11.0 产物结构、commit trailer 格式、`.specpower.yaml` 字段均保持不变
- **行为增强** — 新增 9 个禁止性铁律 R1–R9, 从判定触发器看全部在 v1.11.0 已有检测机制内 (`verify-task-reviews.sh` / 声明缺失检测), 新增 R8/R9 为入口层纪律, 由 SKILL.md 自身强制
- **文档膨胀** — SKILL.md 从 163 行增至约 260 行 (+60%), 换来入口层指令性大幅强化;评估后认为值得 — v1.11.0 的瘦身是为了降低每次加载成本, v1.12.0 的扩容集中在"立即动作 + 冲突覆盖 + 职责划分"三个高频触发点, 不是规则数量膨胀

### 升级指引

从 v1.11.0 升级到 v1.12.0 无破坏性变更。升级动作:

1. `./scripts/link-skill.sh` 重新同步 skill (若使用软链接安装则无需操作)
2. 下次调用 `/spec-power` 时, Claude 将在第一动作看到硬入口清单, 自动按 Step 0–3 执行
3. 若你的项目 `CLAUDE.md` 明确声明 "别用 spec-power" 或 "直接实现不要文档", 行为保持不变 (可降级条件已在覆盖规则中列出)
...
- **"任务完成声明"硬性规则（禁止静默跳过）**
  - 背景：Standard 模式下"diff 低于阈值可跳过子 agent 审查"的默认行为被滥用为"什么都不声明就直接跳过"，导致审查在日常开发中经常被静默省略，质量门控形同虚设
  - SKILL.md "关键规则速查" 新增两条铁律：**禁止静默跳过** 与 **任务完成声明**
  - SKILL.md 新增独立章节 "跳过规则（禁止静默跳过）"：列出可跳过步骤及其前置条件、任务完成声明模板、有效/无效理由对照、违规处置流程
  - 每个任务在 TaskUpdate 标记 completed 前，必须输出结构化的完成声明，显式记录每个检查点的执行/跳过状态和跳过理由（含 diff 行数 + 强制例外检查结论）

- **跳过理由黑名单机制**
  - 无效理由：模糊措辞（"代码简单"、"明显正确"、"不需要审查"）、未说明 diff 行数、未检查强制例外、"最后一起审"、沉默不声明
  - 有效理由示例：具体包含行数 + 强制例外结论，如 "Task 1 diff 仅 8 行，修改 XML 属性，不涉及安全/SQL/并发/金额/公开 API，跳过代码审查"

### 修复 🐛

- **逐任务循环 6 步扩展为 7 步**
  - `phase-guide-execution.md` Phase 6 逐任务循环流程图新增 Step 5.5: "输出任务完成声明"
  - Step 3/Step 4 描述调整为"触发时/声明跳过"，强调两种状态都必须显式表达
  - Step 6 明确提交后先执行 `git log` 验证 commit 存在，再 TaskUpdate 标记完成

- **子 agent 审查触发阈值权威表增加显式声明要求**
  - `phase-guide-execution.md` Phase 6 "子 agent 审查触发阈值（权威表）"下方新增 "显式声明要求" 子章节
  - 列出三个必答问题：diff 多少行、是否涉及强制例外、审查执行或跳过（含具体原因）
  - 明确违规检测规则与处置流程（回退 in_progress、补做审查、后续任务独立提交）

- **SKILL.md "逐任务循环 6 步"一行摘要更新**
  - 原：`实现(TDD) → 自审 → 规范审查(触发时) → 代码审查 → 修复闭环 → git commit`
  - 新：`实现(TDD) → 自审 → 规范审查(触发时/声明跳过) → 代码审查(触发时/声明跳过) → 修复闭环 → 任务完成声明 + git commit`

### 动机

v1.9.0 已将"逐任务 git 提交"升级为铁律，但配套的"逐任务审查"在 Standard 模式下因行数阈值存在"静默跳过"漏洞。本次迭代补齐最后一块——即使决定跳过，也必须把跳过理由写在对话里作为审计留痕。设计哲学：**让跳过成本略高于做审查**，从而在实际执行中反向鼓励做审查。
