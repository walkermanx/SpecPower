...
| 步骤 | 可跳过的前置条件 | 落盘位置 |
|------|----------------|---------|
| Phase 1.5 需求澄清 | 用户需求已包含完整输入/输出/边界/约束;**前端/UI 任务还需:设计稿来源已明确或显式声明无需设计稿** | 提案中加一行说明 |
...
- ✅ "本次为后端 SQL 重构, 不触发 UI 关键词/路径, 设计稿子流程不适用; 已确认输入输出+约束, 跳过 Phase 1.5 澄清"
- ✅ "前端任务但用户已贴 Figma URL 并完成增量读取确认, 视为设计稿来源已固定, 后续仅做范围确认"
