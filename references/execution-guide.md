...
并非所有变更都需要子agent审查。阈值按模式差异化——Strict 模式更保守（用户选择 Strict 意味着任务高风险），Flow 模式最宽松。
...
**简记**: Strict 从 10 行起触发子agent审查；Standard 从 20 行起；Flow 从 100 行起（或不触发）。

#### 强制审查例外

无论 diff 大小和模式，以下变更**必须**触发子agent审查：

- **认证/授权逻辑** — auth, permission, role, session, token 验证
- **加密/哈希** — crypto, hash, secret, key 生成或验证
- **SQL/数据库查询** — 查询构造、schema 变更、迁移脚本
- **正则表达式修改** — ReDoS 风险（尤其是用户输入场景）
- **并发/锁相关** — mutex, semaphore, atomic, race condition 区域
- **金额/计费逻辑** — 价格计算、折扣、支付流程
- **公开 API 签名** — 参数、返回值、错误码变更
- **核心数据结构** — 影响多处消费者的 schema/interface 变更
- **错误处理路径删除** — 移除 try/catch、error boundary、fallback

#### Phase 7 补位职责

Phase 7 全局审查时，必须额外回顾所有在 Phase 6 中跳过子agent审查的小变更，检查：
- 多个小变更组合是否产生集成问题
- 跳过审查的变更是否存在遗漏的例外情况
